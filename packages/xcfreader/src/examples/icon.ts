...
const outDir = path.resolve(__dirname, '../../../../output/icon');
...
(async function main() {
  try {
    ...
    await image.writeImage(path.resolve(outDir, "icon.png"));
    Logger.log(`Rendered icon.xcf (${parser.width}x${parser.height}, ${parser.layers.length} layers)`);
  } catch (err) {
    Logger.error(err);
    process.exit(1);
  }
